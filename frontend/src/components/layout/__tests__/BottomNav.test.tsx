...
describe("BottomNav", () => {
  ...
  it("renders all navigation items with text labels", async () => {
    renderWithRouter("/")
    ...
    expect(bottomNav).toHaveTextContent("Decks")
  ...
  it("each nav item has the correct aria-label", async () => {
    ...
    expect(links).toHaveLength(6) // 6 items after adding Decks in Story 2.2
    ...
    expect(ariaLabels).toContain("Decks — vocabulary organization")
