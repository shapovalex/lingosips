...
class TestCreateCardStream:
    ...
    async def test_field_update_events_emitted_in_order(self, mock_llm, session) -> None:
    ...
    async def test_complete_event_emitted_with_card_id(self, mock_llm, session) -> None:
        """AC1: complete event emitted with integer card_id."""
        from lingosips.core.cards import CardCreateRequest, create_card_stream

        request = CardCreateRequest(target_word="melancólico")
        events = await self._collect_events(create_card_stream(request, mock_llm, session, "es"))

        complete_events = [e for e in events if e["event"] == "complete"]
        assert len(complete_events) == 1
        assert isinstance(complete_events[0]["data"]["card_id"], int)

    async def test_card_persisted_to_db(self, mock_llm, session) -> None:
    ...
    async def test_fsrs_initial_state_set_on_card(self, mock_llm, session) -> None:
    ...
    async def test_llm_timeout_emits_error_event(self, session) -> None:
        ...
        events = await self._collect_events(create_card_stream(request, mock, session, "es"))

        error_events = [e for e in events if e["event"] == "error"]
        assert len(error_events) == 1
        assert error_events[0]["data"]["message"] == "Local Qwen timeout after 10s"

    async def test_safety_blocked_emits_error_no_card(self, mock_llm, session) -> None:
        ...
        with patch("lingosips.core.cards.safety.check_text", return_value=(False, "blocked")):
            events = await self._collect_events(
                create_card_stream(request, mock_llm, session, "es")
            )
    ...
    async def test_invalid_llm_response_emits_error(self, session) -> None:
